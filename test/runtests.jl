...
@testset "trjarray.jl" begin
    ...
    @testset "constructor" begin
        nframe = 5
        natom = 5
        ta = TrjArray{Float64, Int64}(x=rand(nframe, natom), y=rand(nframe, natom), z=rand(nframe, natom))
    ...
    @testset "slicing with boxsize" begin
        nframe = 5
        natom = 5
        ta = TrjArray{Float64, Int64}(x=rand(nframe, natom), y=rand(nframe, natom), z=rand(nframe, natom), boxsize=rand(nframe, 3))
    ...
    @testset "atom selection" begin
        nframe = 5
        natom = 5
        ta = TrjArray{Float64, Int64}(x=rand(nframe, natom), y=rand(nframe, natom), z=rand(nframe, natom), atomid=1:natom, chainname=fill("A", natom))
    ...
    @testset "merging" begin
        ...
        ta_new = [topology; ta]
        @test ta_new["atomid 1:3"].x ≈ ta.x[:, 1:3]
    end
end
...
@testset "structure.jl" begin
    @testset "centerofmass" begin
        c = [-1.0 0.0 1.0;
             -1.0 0.0 1.0]
        ta = TrjArray{Float64, Int64}(x=c, y=c, z=c)
        com = centerofmass(ta)
        @test com.x ≈ com.y ≈ com.z ≈ [0.0, 0.0]

        c = [-2.0 0.0 1.0;
             -2.0 0.0 1.0]
        ta = TrjArray{Float64, Int64}(x=c, y=c, z=c, mass=[0.5, 1.0, 1.0])
        com = centerofmass(ta)
        @test com.x ≈ com.y ≈ com.z ≈ [0.0, 0.0]
